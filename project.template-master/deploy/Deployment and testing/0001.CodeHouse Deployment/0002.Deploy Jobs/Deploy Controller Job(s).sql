/***********************************************************************************************************************************
* Script      : 3.SQL Server Agent - Controller Jobs.sql
...
DECLARE @Environment CHAR(3) = CASE WHEN @@SERVERNAME IN ('CPTDEVDB02','CPTDEVDB10') THEN 'DEV'
                               ...
                               ELSE 'DEV' END;
--IF @Environment = 'DEV' BEGIN;
--  THROW 50000, 'Unknown Environment. Terminating', 1;
--  SET NOEXEC ON;
--END;

DECLARE @ProcessName VARCHAR(150) = 'JobController_01';
DECLARE @ProcessID INT = [Config].[GetProcessIDByName] (@ProcessName);
DECLARE @ProcessJobSchema NVARCHAR(128) = N'DataFlow';
DECLARE @ProcessJobProcedure NVARCHAR(128) = N'JobController';
DECLARE @JobNameClass VARCHAR(30) = 'Controller';
DECLARE @JobCategoryClass VARCHAR(30) = 'Process';
DECLARE @IsLoopJob BIT = 1;
DECLARE @IsControllerJob BIT = 1;
DECLARE @EnableJobLog BIT = 0;
DECLARE @WaitTime VARCHAR(9) = '00:01:00';
DECLARE @CheckServiceBroker BIT = 0;
DECLARE @DeleteExisting BIT = 0;
DECLARE @ProcedureParams NVARCHAR(1000) = '@ControllerProcessID = ' + CAST(@ProcessID AS VARCHAR);
