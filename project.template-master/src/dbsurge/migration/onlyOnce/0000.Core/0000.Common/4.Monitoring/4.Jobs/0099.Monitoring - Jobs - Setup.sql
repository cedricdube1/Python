...
DECLARE @ProcessName NVARCHAR(128) = 'Monitoring|Jobs';
/***********************************************************************************************************************************
-- Process --
***********************************************************************************************************************************/
IF NOT EXISTS (SELECT * FROM [Config].[Process] WHERE [ProcessName] = @ProcessName) BEGIN;
  SET IDENTITY_INSERT [Config].[Process] ON;
  INSERT INTO [Config].[Process] ([ProcessID], [ProcessName], [ProcessDescription], [IsEnabled]) VALUES(-50, @ProcessName, 'Monitor Jobs', 1);
...
--
SET @ProcessTaskID = [Config].[GetProcessTaskByName] (@ProcessID, 'JobFailures');
...
EXEC [Config].[SetVariable_ProcessTask]   @ProcessTaskID = @ProcessTaskID,
                                          ...
                                          @ConfigValue = 60,
                                          @Delete = 0;
SET @ProcessTaskID = [Config].[GetProcessTaskByName] (@ProcessID, 'ControllerJobDisabled');
EXEC [Config].[SetVariable_ProcessTask]   @ProcessTaskID = @ProcessTaskID,
                                          @ConfigGroupName = 'Logging',
                                          @ConfigName = 'InfoDisabled',
                                          @ConfigValue = 1,
