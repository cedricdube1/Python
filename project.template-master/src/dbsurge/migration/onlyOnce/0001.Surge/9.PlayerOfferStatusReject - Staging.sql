/************************************************************************
* Script     : 9.PlayerOfferStatusReject - Staging.sql
...
*            :   1.1 > NONE, uses HubPlayerOffer
* Steps(post): 2 > Procedures
*            :     3.1 > Extract_PlayerOfferStatusReject (handled by Common, no need to specify here)
*            :     3.2 > Stage_PlayerOfferStatusReject
*            :     3.3 > Load_PlayerOfferStatusReject
*            :     3.4 > Process_PlayerOfferStatusReject
*            :   4 > Config
*            :     4.1 > Setup: Process
*            : 3 > Calling Scripts
*            :   3.1 > GetDeployment
...
------------------
-- Stage_PlayerOfferStatusReject
------------------
DECLARE @Layer VARCHAR(50) = 'Staging',
        @Stream VARCHAR(50) = 'PlayerOfferStatusReject',
        @StreamVariant VARCHAR(50) = 'Surge',
        @CodeObjectName NVARCHAR(128) = 'Stage_PlayerOfferStatusReject',
...
'Stage Surge PlayerOfferStatusReject from Delta keys.';
    ...
    BEGIN TRY
      ...
      INSERT INTO #Stage_{StreamVariant}_{Stream} (
        ...
        [MaxTier]
               ...
               ISNULL([PlayerID], -1),
               ISNULL([GamingServerID], -1),
               ISNULL([Surge].[HubPlayerOfferHash] ([GamingServerID], [OfferGUID], [PromoGUID], [PlayerID]),@DefaultHubID) AS [HubPlayerOfferID],
               -- ATTRIBUTES --
               -- Relationships --
               ISNULL([Surge].[HubPlayerHash] ([GamingServerID], [PlayerID]),@DefaultHubID) AS [HubPlayerID],
               ''Reject'',
               ISNULL([PromoType], ''Unknown''),
               -- Dates --
               COALESCE([InsertedDateTime], @NowTime),
               COALESCE([EndDateTime], @NowTime),
               -- Others --
               ISNULL([CurrentTier], -1),
               ISNULL([MaxTiers], -1)
          FROM #ExtractDelta_{StreamVariant}_{Stream} [CT]
         INNER JOIN [{ExtractDatabase}].[{ExtractSchema}].[{ExtractTableName}] [STBL]
            ON [CT].[PayloadID] = [STBL].[OptInCancelID]
...
GO
------------------
-- Load_PlayerOfferStatusReject
------------------
DECLARE @Layer VARCHAR(50) = 'Staging',
        @Stream VARCHAR(50) = 'PlayerOfferStatusReject',
        @StreamVariant VARCHAR(50) = 'Surge',
        @CodeObjectName NVARCHAR(128) = 'Load_PlayerOfferStatusReject',
        @VersionType CHAR(5) = 'Major',
        @ObjectType VARCHAR(50) = 'Procedure',
        @CodeType VARCHAR(50) = 'Task',
        @Author NVARCHAR(128) = 'Cedric Dube',
...
'{<LoadTaskStatement_Header>}
  {<LoadTaskStatement_IterationsHeader>}
    ...
    BEGIN TRANSACTION;
      ...
      INSERT INTO [dbo].[Status] (
        ...
        [StatusName]
               ...
               PLD.[StatusName]
         FROM #Stage_Surge_PlayerOfferStatusReject PLD
    ...
    SET @UpdateCount = 0;
    SET @MergeCount = 0;
    SET @TargetObject = ''dbo.PlayerOfferStatus'';
    -- Info Log Start --
    SET @StepID = @StepID +1;
    SET @InfoMessage = ''Process to tables: '' + @TargetObject;
    IF @InfoLoggingDisabled = 0 EXEC [Logging].[LogProcessTaskInfoStart] @ProcessLogID = @ProcessLogID, 
    ...
    BEGIN TRANSACTION;
      {<ApplockStatement_GetLock>}
      MERGE dbo.PlayerOfferStatus AS Tgt
      USING (SELECT PLD.HubPlayerOfferID,
                    ...
                    ISNULL(PT.PromotionTypeID, -1) AS [PromotionTypeID],
                    PLD.StatusUTCDateTime,
                    CAST(PLD.StatusUTCDateTime AS DATE) AS [StatusUTCDate],
                    PLD.OfferEndUTCDateTime,
                    CAST(PLD.OfferEndUTCDateTime AS DATE) AS [OfferEndUTCDate],
                    PLD.CurrentTier,
                    PLD.MaxTier
               FROM #Stage_Surge_PlayerOfferStatusReject PLD
               LEFT JOIN [dbo].[Status] ST WITH (NOLOCK)
                 ON PLD.[StatusName] = ST.[StatusName]
               LEFT JOIN [dbo].[PromotionType] PT WITH (NOLOCK)
                 ON PLD.[PromotionTypeName] = PT.[PromotionTypeName]
      ...
      WHEN MATCHED AND Src.ModifiedDate >= Tgt.ModifiedDate
        ...
        UPDATE SET
            ...
            Tgt.StatusUTCDateTime = Src.StatusUTCDateTime,
            Tgt.StatusUTCDate = Src.StatusUTCDate,
            Tgt.OfferEndUTCDateTime = Src.OfferEndUTCDateTime,
            Tgt.OfferEndUTCDate = Src.OfferEndUTCDate,
            Tgt.CurrentTier = Src.CurrentTier,
            Tgt.MaxTier = Src.MaxTier
      WHEN NOT MATCHED BY TARGET THEN
        ...
        VALUES (
            ...
            Src.StatusID,
            Src.PromotionTypeID,
            Src.StatusUTCDateTime,
            Src.StatusUTCDate,
            Src.OfferEndUTCDateTime,
            Src.OfferEndUTCDate,
            Src.CurrentTier,
            Src.MaxTier
        );
...
GO
------------------
-- Process_PlayerOfferStatusReject
------------------
DECLARE @Layer VARCHAR(50) = 'Staging',
        @Stream VARCHAR(50) = 'PlayerOfferStatusReject',
        @StreamVariant VARCHAR(50) = 'Surge',
        @CodeObjectName NVARCHAR(128) = 'Process_PlayerOfferStatusReject',
        @VersionType CHAR(5) = 'Major',
        @ObjectType VARCHAR(50) = 'Procedure',
...
'{<ProcessStatement_Header>}
  ...
  CREATE TABLE #Stage_{StreamVariant}_{Stream}(
    ...
    -- Dates --
    [StatusUTCDateTime] [DATETIME2] NOT NULL,
    [OfferEndUTCDateTime] [DATETIME2] NOT NULL,
    -- Others --
...
EXEC [Config].[SetVariable_AppLock] @ConfigName = ''Timeout'',
                                    ...
                                    @ConfigValue = 1000;
EXEC [Config].[SetVariable_AppLock] @ConfigName = ''Retry'',
                                    @ObjectName = ''dbo.PlayerOfferStatus'',
                                    @ConfigValue = 100;
EXEC [Config].[SetVariable_AppLock] @ConfigName = ''Timeout'',
                                    @ObjectName = ''dbo.PlayerOfferStatus'',
                                    @ConfigValue = 1000;
  ...
  IF @StreamVariantTag IS NULL
    ...
    INSERT INTO @GenerateList (
      ...
      [ObjectLayer]
             ...
             (4,  ''PlayerOfferStatusReject'', ''Surge'', ''Procedure'', ''Task'', ''Stage_PlayerOfferStatusReject'', @Layer),
             (5, ''PlayerOfferStatusReject'', ''Surge'', ''Procedure'', ''Task'', ''Load_PlayerOfferStatusReject'', @Layer),
             (6, ''PlayerOfferStatusReject'', ''Surge'', ''Procedure'', ''Process'', ''Process_PlayerOfferStatusReject'', @Layer),
             (7, ''PlayerOfferStatusReject'', ''Surge'', ''Script'',    ''Process'', ''Setup'', @Layer),
