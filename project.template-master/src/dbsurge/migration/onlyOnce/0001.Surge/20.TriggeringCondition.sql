/************************************************************************
* Script     : 14.TriggeringCondition - Staging.sql
...
*            :   1.1 > HubTriggeringConditionHash
* Steps(post): 2 > Procedures
*            :     3.1 > Extract_TriggeringCondition (handled by Common, no need to specify here)
*            :     3.2 > Stage_TriggeringCondition
*            :     3.3 > Load_TriggeringCondition
*            :     3.4 > Process_TriggeringCondition
*            :   4 > Config
*            :     4.1 > Setup: Process
*            : 3 > Calling Scripts
*            :   3.1 > GetDeployment
...
-------------------------------------
------------------
-- HubTriggeringConditionHash
------------------
DECLARE @Layer VARCHAR(50) = 'Staging',
        @Stream VARCHAR(50) = 'TriggeringCondition',
        @StreamVariant VARCHAR(50) = 'Surge',
        @CodeObjectName NVARCHAR(128) = 'HubTriggeringConditionHash',
...
'@GamingSystemID INT,
  @TriggeredUTCDateTime DATETIME2(7),
  @UserID INT';
...
'BEGIN;
  ...
  SET @HashKey = CAST(HASHBYTES(''SHA2_256'',
									   ...
									   +''-''+
                                       TRY_CONVERT(VARCHAR, @TriggeredUTCDateTime, 121)
									   +''-''+
...
-------------------------------------
------------------
-- Stage_TriggeringCondition
------------------
DECLARE @Layer VARCHAR(50) = 'Staging',
        @Stream VARCHAR(50) = 'TriggeringCondition',
        @StreamVariant VARCHAR(50) = 'Surge',
        @CodeObjectName NVARCHAR(128) = 'Stage_TriggeringCondition',
        @VersionType CHAR(5) = 'Major',
...
SET @CodeObjectRemark = 
'Stage Surge TriggeringCondition from Delta keys.';
    ...
    BEGIN TRY
      ...
      INSERT INTO #Stage_{StreamVariant}_{Stream} (
        ...
        [TriggeredUTCDateTime]
               ...
               @ProcessLogID,
               [ID],
               COALESCE([TriggeredDate], @NowTime),
               @SourceSystemID,
               @OriginSystemID,
               -- KEYS --
			   ISNULL([GamingSystemID],-1),
			   ISNULL([UserID],-1),
               ISNULL([Surge].[HubTriggeringConditionHash] ([GamingSystemID], [TriggeredDate], [UserID]),@DefaultHubID) AS [HubTriggeringConditionID],
               -- ATTRIBUTES --
               -- Relationships --
               ISNULL([Surge].[HubPlayerHash] ([GamingSystemID], [UserID]),@DefaultHubID) AS [HubPlayerID],
               ISNULL([Identifier], ''Unknown''),
               ISNULL([GenerateEvent], ''Unknown''),
               ISNULL([Results], ''Unknown''),
               ISNULL([AdditionalData], ''Unknown''),
               -- Dates --
               ISNULL([StartDate], CAST(''1900-01-01'' AS DATETIME2)),
               ISNULL([TriggeredDate], CAST(''1900-01-01'' AS DATETIME2))
...
GO
------------------
-- Load_TriggeringCondition
------------------
DECLARE @Layer VARCHAR(50) = 'Staging',
        @Stream VARCHAR(50) = 'TriggeringCondition',
        @StreamVariant VARCHAR(50) = 'Surge',
        @CodeObjectName NVARCHAR(128) = 'Load_TriggeringCondition',
        @VersionType CHAR(5) = 'Major',
		@ObjectType VARCHAR(50) = 'Procedure',
		@CodeType VARCHAR(50) = 'Task',
        @Author NVARCHAR(128) = 'Cedric Dube',
...
'{<LoadTaskStatement_Header>}
  {<LoadTaskStatement_IterationsHeader>}
    ...
    SET @UpdateCount = 0;
    SET @MergeCount = 0;
    SET @TargetObject = ''dbo.Identifier'';
    -- Info Log Start --
    SET @StepID = @StepID +1;
    ...
    BEGIN TRANSACTION;
      ...
      INSERT INTO [dbo].[Identifier] (
        ...
        [IdentifierName]
               ...
               PLD.[IdentifierName]
         FROM #Stage_Surge_TriggeringCondition PLD
         LEFT JOIN [dbo].[Identifier] GM
           ON PLD.[IdentifierName] = GM.[IdentifierName]
         WHERE GM.[IdentifierID] IS NULL
    ...
    BEGIN TRANSACTION;
      ...
      INSERT INTO [dbo].[Event] (
        ...
        [EventName]
               ...
               PLD.[EventName]
         FROM #Stage_Surge_TriggeringCondition PLD
         LEFT JOIN [dbo].[Event] RG
           ON PLD.[EventName] = RG.[EventName]
         WHERE RG.[EventID] IS NULL
          AND PLD.[StageID] >= @ProcessBatchBeginID AND PLD.[StageID] < @ProcessBatchEndID
         GROUP BY PLD.[CaptureLogID],
                  PLD.[EventName];
      SET @InsertCount = @@ROWCOUNT;
    ...
    SET @UpdateCount = 0;
    SET @MergeCount = 0;
    SET @TargetObject = ''dbo.TriggerResult'';
    -- Info Log Start --
    SET @StepID = @StepID +1;
    SET @InfoMessage = ''Process to tables: '' + @TargetObject;
    IF @InfoLoggingDisabled = 0 EXEC [Logging].[LogProcessTaskInfoStart] @ProcessLogID = @ProcessLogID, 
    ...
    BEGIN TRANSACTION;
      ...
      ;WITH CTE AS (
       SELECT PLD.[CaptureLogID],
              ...
              PLD.[TriggerResults],
              PLD.[AdditionalData],
              ROW_NUMBER() OVER (PARTITION BY PLD.[TriggerResults] ORDER BY PLD.[CaptureLogID] DESC, PLD.[ModifiedDate] DESC, PLD.[AdditionalData] DESC) AS [RowNo]
         FROM #Stage_Surge_TriggeringCondition PLD
        WHERE PLD.[StageID] >= @ProcessBatchBeginID AND PLD.[StageID] < @ProcessBatchEndID
      )
	  MERGE [dbo].[TriggerResult] AS Tgt
      USING (SELECT [CaptureLogID],
                    [ModifiedDate],
                    [TriggerResults] AS [TriggerResults],
                    [AdditionalData] AS [AdditionalData]
               FROM CTE WHERE [RowNo] = 1) AS Src
        ON Tgt.[TriggerResults] = Src.[TriggerResults]
      WHEN MATCHED AND EXISTS (
            SELECT Src.AdditionalData
            EXCEPT
            SELECT Tgt.AdditionalData
      ) AND (Src.ModifiedDate >= Tgt.ModifiedDate OR tgt.AdditionalData = ''Unknown'') 
        THEN
        UPDATE SET
            Tgt.CaptureLogID = Src.CaptureLogID,
            Tgt.Operation = ''U'',
            Tgt.ModifiedDate = Src.ModifiedDate,
            Tgt.AdditionalData = Src.AdditionalData
    ...
    ----------------------------------------------------------------
    ----------------------------------
    -- HUB TriggeringCondition
    ----------------------------------
    SET TRANSACTION ISOLATION LEVEL READ COMMITTED;
    SET @InsertCount = 0;
    SET @UpdateCount = 0;
    SET @MergeCount = 0;
    SET @TargetObject = ''dbo.HubTriggeringCondition'';
    -- Info Log Start --
    SET @StepID = @StepID +1;
    SET @InfoMessage = ''Process to tables: '' + @TargetObject;
    IF @InfoLoggingDisabled = 0 EXEC [Logging].[LogProcessTaskInfoStart] @ProcessLogID = @ProcessLogID, 
    ...
    BEGIN TRANSACTION;
      ...
      INSERT INTO dbo.HubTriggeringCondition (
        ...
        UserID,
        TriggeredUTCDateTime
      ) SELECT DISTINCT PLD.HubTriggeringConditionID,
                        PLD.SourceSystemID,
                        PLD.ModifiedDate,
                        PLD.OriginSystemID,
                        PLD.CaptureLogID,
                        PLD.GamingSystemID,
                        PLD.UserID,
                        PLD.TriggeredUTCDateTime
        FROM #Stage_Surge_TriggeringCondition PLD
        LEFT JOIN dbo.HubTriggeringCondition Hub
          ON PLD.HubTriggeringConditionID = Hub.HubTriggeringConditionID
         AND PLD.SourceSystemID = Hub.SourceSystemID
        WHERE Hub.HubTriggeringConditionID IS NULL
         AND PLD.[StageID] >= @ProcessBatchBeginID AND PLD.[StageID] < @ProcessBatchEndID;
      SET @InsertCount = @@ROWCOUNT;
      -- RELEASE THE APP. LOCK --
      EXEC [sp_ReleaseAppLock] @Resource = @TargetObject; 
    ...
    SET @UpdateCount = 0;
    SET @MergeCount = 0;
    SET @TargetObject = ''dbo.TriggeringCondition'';
    -- Info Log Start --
    SET @StepID = @StepID +1;
    SET @InfoMessage = ''Process to tables: '' + @TargetObject;
    IF @InfoLoggingDisabled = 0 EXEC [Logging].[LogProcessTaskInfoStart] @ProcessLogID = @ProcessLogID, 
    ...
    BEGIN TRANSACTION;
      {<ApplockStatement_GetLock>}
      MERGE dbo.TriggeringCondition AS Tgt
      USING (SELECT PLD.HubTriggeringConditionID,
                    PLD.SourceSystemID,
                    PLD.OriginSystemID,
                    PLD.CaptureLogID,
                    PLD.ModifiedDate,
                    PLD.HubPlayerID,
                    ISNULL(GM.IdentifierID, -1) AS [IdentifierID],
                    ISNULL(RG.EventID, -1) AS [EventID],
                    ISNULL(TT.TriggerResultID, -1) AS [TriggerResultID],
                    StartUTCDateTime,
                    CAST(StartUTCDateTime AS DATE) AS [StartUTCDate],
                    TriggeredUTCDateTime,
                    CAST(TriggeredUTCDateTime AS DATE) AS [TriggeredUTCDate]
               FROM #Stage_Surge_TriggeringCondition PLD
               LEFT JOIN [dbo].[Identifier] GM WITH (NOLOCK)
                 ON PLD.[IdentifierName] = GM.[IdentifierName]
               LEFT JOIN [dbo].[Event] RG WITH (NOLOCK)
                 ON PLD.[EventName] = RG.[EventName]               
               LEFT JOIN [dbo].[TriggerResult] TT WITH (NOLOCK)
                 ON PLD.[TriggerResults] = TT.[TriggerResults]               
              WHERE PLD.[StageID] >= @ProcessBatchBeginID AND PLD.[StageID] < @ProcessBatchEndID
            )AS Src
        ON Tgt.HubTriggeringConditionID = Src.HubTriggeringConditionID
      ...
      WHEN MATCHED AND Src.ModifiedDate >= Tgt.ModifiedDate
        ...
        UPDATE SET
            ...
            Tgt.IdentifierID = Src.IdentifierID,
            Tgt.EventID = Src.EventID,
            Tgt.TriggerResultID = Src.TriggerResultID,
            Tgt.StartUTCDateTime = Src.StartUTCDateTime,
            Tgt.StartUTCDate = Src.StartUTCDate,
            Tgt.TriggeredUTCDateTime = Src.TriggeredUTCDateTime,
            Tgt.TriggeredUTCDate = Src.TriggeredUTCDate
      WHEN NOT MATCHED BY TARGET THEN
        INSERT (
            HubTriggeringConditionID,
            SourceSystemID,
        ...
        VALUES (
            ...
            Src.StartUTCDate,
            Src.TriggeredUTCDateTime,
            Src.TriggeredUTCDate
        );
      SET @MergeCount = @@ROWCOUNT;
      -- RELEASE THE APP. LOCK --
      EXEC [sp_ReleaseAppLock] @Resource = @TargetObject; 
...
GO
------------------
-- Process_TriggeringCondition
------------------
DECLARE @Layer VARCHAR(50) = 'Staging',
        @Stream VARCHAR(50) = 'TriggeringCondition',
        @StreamVariant VARCHAR(50) = 'Surge',
        @CodeObjectName NVARCHAR(128) = 'Process_TriggeringCondition',
        @VersionType CHAR(5) = 'Major',
		@ObjectType VARCHAR(50) = 'Procedure',
		@CodeType VARCHAR(50) = 'Process',
        @Author NVARCHAR(128) = 'Cedric Dube',
...
'{<ProcessStatement_Header>}
  ...
  CREATE TABLE #Stage_{StreamVariant}_{Stream}(
    ...
    INDEX [CIDX_Stage_{StreamVariant}_{Stream}] CLUSTERED (HubTriggeringConditionID ASC, [SourceSystemID] ASC) WITH (FILLFACTOR = 100),
    -- ATTRIBUTES --
    -- Relationships --
    ...
    [TriggeredUTCDateTime] [DATETIME2] NOT NULL,
    -- Others --
    [RowNo] [INT] NULL
  );
...
EXEC [Config].[SetVariable_AppLock] @ConfigName = ''Retry'',
                                    @ObjectName = ''dbo.Event'',
                                    @ConfigValue = 100;
EXEC [Config].[SetVariable_AppLock] @ConfigName = ''Timeout'',
                                    @ObjectName = ''dbo.Event'',
                                    @ConfigValue = 1000;
EXEC [Config].[SetVariable_AppLock] @ConfigName = ''Retry'',
                                    @ObjectName = ''dbo.Identifier'',
                                    @ConfigValue = 100;
EXEC [Config].[SetVariable_AppLock] @ConfigName = ''Timeout'',
                                    @ObjectName = ''dbo.Identifier'',
                                    @ConfigValue = 1000;
EXEC [Config].[SetVariable_AppLock] @ConfigName = ''Retry'',
                                    @ObjectName = ''dbo.TriggerResult'',
                                    @ConfigValue = 100;
EXEC [Config].[SetVariable_AppLock] @ConfigName = ''Timeout'',
                                    @ObjectName = ''dbo.TriggerResult'',
                                    @ConfigValue = 1000;
EXEC [Config].[SetVariable_AppLock] @ConfigName = ''Retry'',
                                    @ObjectName = ''dbo.TriggeringCondition'',
                                    @ConfigValue = 100;
EXEC [Config].[SetVariable_AppLock] @ConfigName = ''Timeout'',
                                    @ObjectName = ''dbo.TriggeringCondition'',
                                    @ConfigValue = 1000;
...
'BEGIN TRY;
  ...
  IF @StreamVariantTag IS NULL
    ...
    INSERT INTO @GenerateList (
      ...
      [CodeObjectName],
	  [ObjectLayer]
    ) VALUES (1, ''TriggeringCondition'', ''Surge'', ''Function'',  ''Process'', ''HubTriggeringConditionHash'', @Layer),
             ...
             (5, ''TriggeringCondition'', ''Surge'', ''Procedure'', ''Task'', ''Stage_TriggeringCondition'', @Layer),
             (6, ''TriggeringCondition'', ''Surge'', ''Procedure'', ''Task'', ''Load_TriggeringCondition'', @Layer),
             (7, ''TriggeringCondition'', ''Surge'', ''Procedure'', ''Process'', ''Process_TriggeringCondition'', @Layer),
             (8, ''TriggeringCondition'', ''Surge'', ''Script'',    ''Process'', ''Setup'', @Layer),
