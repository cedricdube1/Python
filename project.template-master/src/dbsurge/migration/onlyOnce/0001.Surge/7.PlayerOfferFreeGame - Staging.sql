/************************************************************************
* Script     : 7.PlayerOfferFreeGame - Staging.sql
...
*            :     3.1 > Extract_PlayerOfferFreeGame (handled by Common, no need to specify here)
*            :     3.2 > Stage_PlayerOfferFreeGame
*            :     3.3 > Load_PlayerOfferFreeGame
*            :     3.4 > Process_PlayerOfferFreeGame
*            :   4 > Config
*            :     4.1 > Setup: Process
*            : 3 > Calling Scripts
*            :   3.1 > GetDeployment
...
------------------
-- Stage_PlayerOfferFreeGame
------------------
DECLARE @Layer VARCHAR(50) = 'Staging',
        @Stream VARCHAR(50) = 'PlayerOfferFreeGame',
        @StreamVariant VARCHAR(50) = 'Surge',
        @CodeObjectName NVARCHAR(128) = 'Stage_PlayerOfferFreeGame',
...
'Stage Surge PlayerOfferFreeGame from Delta keys.';
    ...
    BEGIN TRY
      ...
      INSERT INTO #Stage_{StreamVariant}_{Stream} (
        ...
        [NumberOfSpins]
			   ...
			   ISNULL([FreeGameName], ''Unknown''),
               ISNULL([Trigger], ''Unknown''),
...
GO
------------------
-- Load_PlayerOfferFreeGame
------------------
DECLARE @Layer VARCHAR(50) = 'Staging',
        @Stream VARCHAR(50) = 'PlayerOfferFreeGame',
        @StreamVariant VARCHAR(50) = 'Surge',
        @CodeObjectName NVARCHAR(128) = 'Load_PlayerOfferFreeGame',
        @VersionType CHAR(5) = 'Major',
		@ObjectType VARCHAR(50) = 'Procedure',
		@CodeType VARCHAR(50) = 'Task',
        @Author NVARCHAR(128) = 'Cedric Dube',
...
'{<LoadTaskStatement_Header>}
  {<LoadTaskStatement_IterationsHeader>}
    ...
    BEGIN TRANSACTION;
      ...
      INSERT INTO [dbo].[Game] (
        ...
        [GameName]
               ...
               PLD.[GameName]
         FROM #Stage_Surge_PlayerOfferFreeGame PLD
    ...
    BEGIN TRANSACTION;
      ...
      INSERT INTO [dbo].[Trigger] (
        ...
        [TriggerName]
               ...
               PLD.[TriggerName]
         FROM #Stage_Surge_PlayerOfferFreeGame PLD
         LEFT JOIN [dbo].[Trigger] TG
           ON PLD.[TriggerName] = TG.[TriggerName]
         WHERE TG.[TriggerID] IS NULL
          AND PLD.[StageID] >= @ProcessBatchBeginID AND PLD.[StageID] < @ProcessBatchEndID
         GROUP BY PLD.[CaptureLogID],
                  PLD.[TriggerName];
      SET @InsertCount = @@ROWCOUNT;
    ...
    SET @UpdateCount = 0;
    SET @MergeCount = 0;
    SET @TargetObject = ''dbo.PlayerOfferFreeGame'';
    -- Info Log Start --
    SET @StepID = @StepID +1;
    SET @InfoMessage = ''Process to tables: '' + @TargetObject;
    IF @InfoLoggingDisabled = 0 EXEC [Logging].[LogProcessTaskInfoStart] @ProcessLogID = @ProcessLogID, 
    ...
    BEGIN TRANSACTION;
      {<ApplockStatement_GetLock>}
      MERGE dbo.PlayerOfferFreeGame AS Tgt
      ...
      WHEN MATCHED AND Src.ModifiedDate >= Tgt.ModifiedDate
        ...
        UPDATE SET
            ...
            Tgt.TriggerID = Src.TriggerID,
            Tgt.PromotionTypeID = Src.PromotionTypeID,
            Tgt.NumberOfSpins = Src.NumberOfSpins
...
GO
------------------
-- Process_PlayerOfferFreeGame
------------------
DECLARE @Layer VARCHAR(50) = 'Staging',
        @Stream VARCHAR(50) = 'PlayerOfferFreegame',
        @StreamVariant VARCHAR(50) = 'Surge',
        @CodeObjectName NVARCHAR(128) = 'Process_PlayerOfferFreeGame',
        @VersionType CHAR(5) = 'Major',
		@ObjectType VARCHAR(50) = 'Procedure',
...
EXEC [Config].[SetVariable_AppLock] @ConfigName = ''Timeout'',
                                    ...
                                    @ConfigValue = 1000;
EXEC [Config].[SetVariable_AppLock] @ConfigName = ''Retry'',
                                    @ObjectName = ''dbo.PlayerOfferFreeGame'',
                                    @ConfigValue = 100;
EXEC [Config].[SetVariable_AppLock] @ConfigName = ''Timeout'',
                                    @ObjectName = ''dbo.PlayerOfferFreeGame'',
                                    @ConfigValue = 1000;
  ...
  IF @StreamVariantTag IS NULL
    ...
    INSERT INTO @GenerateList (
	  ...
	  [ObjectLayer]
             ...
             (4,  ''PlayerOfferFreeGame'', ''Surge'', ''Procedure'', ''Task'', ''Stage_PlayerOfferFreeGame'', @Layer),
             (5, ''PlayerOfferFreeGame'', ''Surge'', ''Procedure'', ''Task'', ''Load_PlayerOfferFreeGame'', @Layer),
             (6, ''PlayerOfferFreeGame'', ''Surge'', ''Procedure'', ''Process'', ''Process_PlayerOfferFreeGame'', @Layer),
             (7, ''PlayerOfferFreeGame'', ''Surge'', ''Script'',    ''Process'', ''Setup'', @Layer),
